...
- `que-es-la-uf` migra cuando exista `/guias/uf-costo-de-vida/` y al menos un activo satélite o herramienta adicional alineada que permita tratar UF como núcleo propio de costo de vida sin depender del hub de deuda como entrada principal.
- `reajuste-arriendo-uf-ipc-chile` migra cuando exista `/guias/uf-costo-de-vida/` y el frente pueda ordenar de forma productiva UF, IPC, reajustes y costo de vida sin seguir usando `deuda-credito` como casa operativa de transición.
- `que-es-el-ipc-chile-como-se-calcula` migra cuando exista `/guias/uf-costo-de-vida/` y al menos un satélite o herramienta adicional que ordene IPC, UF y costo de vida.
...
| `que-es-la-uf` | `cluster: deuda-credito`, `category: deuda-credito` | `uf-costo-de-vida` | `deuda-credito` | transitoria | no tratarlo como core de `deuda-credito`; usarlo solo como bridge `related` cuando la duda nace desde cuotas, arriendos u obligaciones reajustables | documentar transición en policy + registry + auditoría; demoverlo a `related` en el hub vivo de deuda | migrar cuando exista `/guias/uf-costo-de-vida/` y un segundo activo satélite o herramienta alineada | La necesidad dominante es entender UF, inflación y reajustes de bolsillo/contratos; crédito solo aporta un caso de uso relacionado |
| `reajuste-arriendo-uf-ipc-chile` | `cluster: deuda-credito`, `category: general` | `uf-costo-de-vida` | `deuda-credito` | transitoria | no tratarlo como core de `deuda-credito`; usarlo como explainer satélite mientras no exista el hub propio de UF/costo de vida | documentar transición en policy + registry + auditoría; enlazarlo desde activos UF/IPC/calculadora sin inflar el hub de deuda | migrar cuando exista `/guias/uf-costo-de-vida/` y la arquitectura productiva pueda absorber explainers + herramientas del frente | La necesidad dominante es entender cómo se aplica una cláusula de reajuste de arriendo en UF o IPC y cuánto impacta el pago, no resolver un problema de crédito |
| `que-es-el-ipc-chile-como-se-calcula` | `cluster: empleo-ingresos`, `category: general` | `uf-costo-de-vida` | `empleo-ingresos` | transitoria | no tratarlo como core de `empleo-ingresos`; usarlo como puente lateral hacia futuros activos UF/costo de vida | documentar transición en policy + registry + auditoría; no inventar owner actual falso | migrar cuando exista `/guias/uf-costo-de-vida/` y un segundo activo satélite o herramienta alineada | La intención dominante es inflación, reajuste y bolsillo; no continuidad de ingreso laboral |
