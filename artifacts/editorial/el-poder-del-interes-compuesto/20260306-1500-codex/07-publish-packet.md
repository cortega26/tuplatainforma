...
- `pnpm run check:editorial` -> pass (con warnings preexistentes del repo en `check:editorial-artifacts`)
- `pnpm run check:dialect` -> pass
- `node scripts/check-hero-images.mjs --require-public-path` -> pass
- `node scripts/check-hero-prompts.mjs` -> pass
